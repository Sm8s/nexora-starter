...
NEXT_PUBLIC_SUPABASE_ANON_KEY=<anon_key>`}
  ...
  const onMagic = async () => {
    ...
    setLoading(false);
    if (error) return setError(error.message);
    alert('Magic Link gesendet. Check deine E-Mail.');
  };

  return (
    <div className="max-w-md mx-auto card">
      <h2 className="text-xl font-semibold mb-2">Einloggen</h2>
      ...
      <div className="mt-4 flex items-center gap-2">
        <button onClick={onMagic} className="text-sm underline">Magic Link senden</button>
        <span className="opacity-60 text-sm">·</span>
        <Link href="/register" className="text-sm underline">Account erstellen</Link>
      </div>
    </div>
  );
}
